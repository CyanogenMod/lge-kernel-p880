/*
 * arch/arm/mach-tegra/board-lx-power.c
 *
...
/*
 * should be checked. LX don't use micro SD
 */
...
static struct i2c_board_info __initdata lx_regulators[] = {
#if defined(CONFIG_REGULATOR_TPS80031)
	{
...
GREG_INIT(2, ldo_vdd_fuse_3v3_en, NULL, VFUSE_LDO_EN, false, 0, 0, 0, 0);
...
static int __init lx_gpio_switch_regulator_init(void)
{
	int i;
...
static struct i2c_board_info __initdata lx_aat2870_i2c_board_info[] = {
	{
...
int __init lx_regulator_init(void)
{
	...
	i2c_register_board_info(4, lx_regulators,
			ARRAY_SIZE(lx_regulators));

#if defined(CONFIG_REGULATOR_AAT2870)
	i2c_register_board_info(2, lx_aat2870_i2c_board_info,
			ARRAY_SIZE(lx_aat2870_i2c_board_info));
#endif

#if defined(CONFIG_REGULATOR_GPIO_SWITCH)
	lx_gpio_switch_regulator_init();
#endif
	pm_power_off = lx_power_off;
	return 0;
}

static void lx_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void lx_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data lx_suspend_data = {
	.cpu_timer	= 2000,
	.cpu_off_timer	= 200,
...
//	.wake_enb	  = TEGRA_WAKE_GPIO_PV0,
	.board_suspend = lx_board_suspend,
	.board_resume = lx_board_resume,
};

static void lx_init_deep_sleep_mode(void)
{
#if defined(CONFIG_MACH_LX)
	lx_suspend_data.suspend_mode = TEGRA_SUSPEND_LP0;
#else
	struct board_info bi;
	tegra_get_board_info(&bi);
	if (bi.board_id == BOARD_1205 && bi.fab == LX_FAB_A01)
		lx_suspend_data.suspend_mode = TEGRA_SUSPEND_LP1;
#endif
}

int __init lx_suspend_init(void)
{
	lx_init_deep_sleep_mode();
	tegra_init_suspend(&lx_suspend_data);
...
static struct platform_device lx_bpc_mgmt_device = {
	.name		= "tegra-bpc-mgmt",
	.id		= -1,
	.dev		= {
...
void __init lx_bpc_mgmt_init(void)
{
...
#endif
	platform_device_register(&lx_bpc_mgmt_device);
...
int __init lx_power_late_init(void)
{
	int irq;
	int ret;
...
late_initcall(lx_power_late_init);
