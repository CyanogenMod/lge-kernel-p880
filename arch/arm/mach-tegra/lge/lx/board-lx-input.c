/*
 * arch/arm/mach-tegra/lge/board-lx-input.c
 *
...
struct touch_operation_role touch_role = {
	...
	.booting_delay 		= 100,
	.reset_delay		= 20,
	.report_period		= 10000000, 	/* 12.5 msec -> 10.0 msec(LX) */
...
static int prox_ldo_en = TEGRA_GPIO_PD4;

uint32_t prox_pwr_mask = 0; //don't need but

static s32 lx_apds990x_power_init(void)
{
        s32 ret = 0;
	printk(KERN_INFO"[SENSOR-APDS] %s start!![%d] \n",__func__,__LINE__);
...
static s32 lx_apds990x_irq_set(void)
{
	s32 ret = 0;
...
static s32 lx_mpuirq_init(void)
{
	s32 ret = 0;
...
struct apds990x_proximity_platform_data lx_prox_data = {
        .gpio		= TEGRA_GPIO_PK2,
...
#include <linux/mpu.h>
#if defined(CONFIG_MACH_X3_REV_A) || defined(CONFIG_MACH_X3_REV_B)
...
#elif defined(CONFIG_MACH_LX_REV_A)
struct mpu_platform_data mpu6050_data = {
        .int_config      = 0x10,
        .level_shifter 	 = 0,
        .orientation     = {  -1,  0,  0,
...
int __init lx_sensor_input_init(void)
{
