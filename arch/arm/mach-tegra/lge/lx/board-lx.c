...
extern void lx_i2c_init(void);
extern int lx_baseband_init(void);
#if defined (CONFIG_REBOOT_MONITOR)
extern void lx_setup_reboot(void);
#else
...
static __initdata struct tegra_clk_init_table lx_clk_init_table[] = {
...
static struct platform_device *lx_uart_devices[] __initdata = {
	&tegra_uarta_device,
	&tegra_uartb_device,
	&tegra_uartc_device,
	&tegra_uartd_device,
...
static struct tegra_uart_platform_data lx_uart_pdata;
static struct tegra_uart_platform_data lx_loopback_uart_pdata;
static void __init uart_debug_init(void)
{
	...
	lx_uart_devices[3] = &debug_uartd_device;
...
static void __init lx_uart_init(void)
{
	int i;
	struct clk *c;
	...
	lx_uart_pdata.parent_clk_list = uart_parent_clk;
	lx_uart_pdata.parent_clk_count = ARRAY_SIZE(uart_parent_clk);
	lx_loopback_uart_pdata.parent_clk_list = uart_parent_clk;
	lx_loopback_uart_pdata.parent_clk_count =
						ARRAY_SIZE(uart_parent_clk);
	lx_loopback_uart_pdata.is_loopback = true;
	tegra_uarta_device.dev.platform_data = &lx_uart_pdata;
	tegra_uartb_device.dev.platform_data = &lx_uart_pdata;
	tegra_uartc_device.dev.platform_data = &lx_uart_pdata;
	tegra_uartd_device.dev.platform_data = &lx_uart_pdata;
	/* UARTE is used for loopback test purpose */
	tegra_uarte_device.dev.platform_data = &lx_loopback_uart_pdata;

	/* Register low speed only if it is selected */
	if (!is_tegra_debug_uartport_hs())
		uart_debug_init();

	platform_add_devices(lx_uart_devices,
				ARRAY_SIZE(lx_uart_devices));
}
...
static struct platform_device *lx_audio_devices[] __initdata = {
    &tegra_ahub_device,
    &tegra_dam_device0,
    &tegra_dam_device1,
    &tegra_dam_device2,
...
static void lx_audio_init(void)
{
    platform_add_devices(lx_audio_devices, ARRAY_SIZE(lx_audio_devices));
}

int lx_read_misc(int index, char* buf, int size)
{
    int h_file = 0;
    int ret = 0;
    int offset = 0;
...
int get_misc_msg(misc_msg_type msg, char* misc_msg, int size)
{
	...
	ret = lx_read_misc(msg, misc_msg, size);
...
int set_misc_msg(misc_msg_type msg, char* misc_msg, int size)
{
	...
	ret = lx_write_misc(msg, misc_msg, size);
...
static void __init tegra_lx_reserve(void)
{
...
MACHINE_START(LX, "lx")
	...
	.reserve        = tegra_lx_reserve,
	.init_early	= tegra_init_early,
	.init_irq       = tegra_init_irq,
	.timer          = &tegra_timer,
	.init_machine   = tegra_lx_init,
MACHINE_END
