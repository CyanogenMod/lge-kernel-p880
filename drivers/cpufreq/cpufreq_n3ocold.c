/*
 *  drivers/cpufreq/cpufreq_n3ocold.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_N3OCOLD
static
#endif
struct cpufreq_governor cpufreq_gov_n3ocold = {
	.name                   = "n3ocold",
...
int cpufreq_n3ocold_cpu_lock(int num_core)
{
	int prev_lock;
...
int cpufreq_n3ocold_cpu_unlock(int num_core)
{
...
void cpufreq_n3ocold_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpu_dbs_info_s *dbs_info;
...
void cpufreq_n3ocold_min_cpu_unlock(void)
{
...
/* cpufreq_n3ocold Governor Tunables */
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_n3ocold_min_cpu_unlock();
	else
		cpufreq_n3ocold_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_n3ocold_cpu_unlock(prev_lock);
	...
	ret = cpufreq_n3ocold_cpu_lock(input);
	if (ret) {
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dvfs_workqueue = create_workqueue("kn3ocold");
	if (!dvfs_workqueue) {
	...
	ret = cpufreq_register_governor(&cpufreq_gov_n3ocold);
	if (ret)
		goto err_reg;
...
{
	cpufreq_unregister_governor(&cpufreq_gov_n3ocold);
	destroy_workqueue(dvfs_workqueue);
	kfree(hotplug_history);
	kfree(rq_data);
}
