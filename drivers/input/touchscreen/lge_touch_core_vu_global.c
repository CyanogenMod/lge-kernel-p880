...
int ghost_finger_solution(struct lge_touch_data *ts)
{
	if(ts->gf_ctrl.stage & GHOST_STAGE_1){
		if(ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0 && ts->ts_data.palm == 0){
		...
		else if(ts->ts_data.total_num == 1 && ts->ts_data.curr_button.state == 0 && ts->ts_data.curr_data[0].id == 0 && ts->ts_data.palm == 0){
			...
			if (unlikely(touch_debug_mask & DEBUG_GHOST) && ts->gf_ctrl.count != ts->gf_ctrl.max_count)
				TOUCH_INFO_MSG("ghost_stage_1: Not good condition. total[%d] button[%d] id[%d] palm[%d]\n",
						ts->ts_data.total_num, ts->ts_data.curr_button.state, ts->ts_data.curr_data[0].id, ts->ts_data.palm);
			ts->gf_ctrl.count = ts->gf_ctrl.max_count;
		}
	}
	...
	ts->gf_ctrl.saved_last_x = ts->ts_data.curr_data[0].x_position;
	ts->gf_ctrl.saved_last_y = ts->ts_data.curr_data[0].y_position;
...
//                                                                                                                  
				if(get_test_mode()==1)
				{
					write_gkpd_value(ts->ts_data.curr_button.key_code);
				}
				else
				{
//                                                                                                                  
...
static void touch_fw_upgrade_func(struct work_struct *work_fw_upgrade)
{
	...
	#if defined(CONFIG_MACH_VU10)
	if((config_id_ver==CONFIG_ID_FORMAL_VER) && (config_id < 100))//Rev.A Board
		{
			if((config_id < fw_image_config_id) && (fw_image_config_id <100))
				b_need_upgrade = 1;
			else
				b_need_upgrade = 0;
		}
	else if((config_id_ver == fw_image_config_id_ver) && (config_id < fw_image_config_id))
		b_need_upgrade = 1;
	#else
...
static ssize_t show_fw_info(struct lge_touch_data *ts, char *buf)
{
	...
	ret += sprintf(buf+ret, "Customer Family  = %d\n", ts->fw_info.customer_family);
	#endif
	return ret;
}
