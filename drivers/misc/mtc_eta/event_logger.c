...
extern int hub_start_touch_logging(void);
extern int hub_start_key_logging(void);
extern void hub_stop_touch_logging(void);
extern void hub_stop_key_logging(void);
	...
	if (!list_empty(&mtc_eta_event_log_q)) {
		...
		switch(event_log->log->id) {
			case MTC_ETA_LOG_ID_KEY:
				PDEBUG("getting key hold %d\n",
						event_log->log->data.key.hold);
				PDEBUG("getting key code 0x%x\n", 
						event_log->log->data.key.key_code);
				break;
			case MTC_ETA_LOG_ID_TOUCH:
				PDEBUG("getting touch action 0x%x\n", 
						event_log->log->data.touch.action);
				PDEBUG("getting touch x %d\n",
						event_log->log->data.touch.x);
				PDEBUG("getting touch y %d\n",
						event_log->log->data.touch.y);
				break;
			default:
				PDEBUG("wrong event\n");
		}
...
static int  __devinit mtc_eta_logger_probe(struct platform_device *pdev)
{
	return misc_register(&mtc_eta_log_dev);
}

static int __devexit mtc_eta_logger_remove(struct platform_device *pdev)
{
	return 0;
}
...
static struct platform_driver mtc_eta_logger_driver = {
	...
	.remove = __devexit_p(mtc_eta_logger_remove),
};
