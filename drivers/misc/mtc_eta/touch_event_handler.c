...
static int mtc_eta_event_log_connect(struct input_handler *handler, struct input_dev *dev, const struct input_device_id *id)
{
	...
	if (strcmp(dev->name, "touch_dev") != 0)
		return 0;
		...
		case EV_ABS:
			switch (code) {
				...
				case ABS_MT_POSITION_X:
					...
					//for Cosmo
					//touch_event.x = value * 1000 / 2334; // 2.334 = 1120(max X)/480(width)
					//for X3
					touch_event.x = value * 1000 / 1527; // 1.527 = 1100(max X)/720(width)
#ifdef MTC_ETA_DEBUG
					if (touch_state != TOUCH_STATE_DOWN) {
						PDEBUG("Wrong touch state in ABS_MT_POSITION_X\n");
					}
#endif
					break;
				case ABS_MT_POSITION_Y:
					...
					//for X3
					touch_event.y = value * 1000 / 1367; // 1.367 = 1750(max Y)/1280(height)
#ifdef MTC_ETA_DEBUG
					if (touch_state != TOUCH_STATE_DOWN) {
						PDEBUG("Wrong touch state in ABS_MT_POSITION_Y\n");
					}
#endif
					break;
			}
