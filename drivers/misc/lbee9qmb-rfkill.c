...
static void update_host_wake_locked(void *data)
{
	...
	if(lpm->active_low){
		if (!h_wake) {
			dev_dbg(lpm->dev, "%s: Call host_wake_lock_timeout\n", __func__);
			wake_lock_timeout(&lpm->host_wake_lock,
		...
		if (h_wake) {
			...
			dev_dbg(lpm->dev, "%s: Expired host_wake_lock_timeout\n", __func__);
	}
}
...
static int lbee9qmb_rfkill_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&lpm->host_wake_lock, WAKE_LOCK_SUSPEND, "BTLowPower_HOSTWAKE");
