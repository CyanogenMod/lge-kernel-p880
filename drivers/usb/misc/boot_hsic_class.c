...
static void modem_read_bulk_callback(struct urb *urb)
{
	...
	if (urb->status) {
		...
		printk(KERN_DEBUG "modem_read_bulk_callback, length = %d",urb->actual_length);
	}
	dev->ongoing_read = 0;
			       ...
			       loff_t *ppos)
{
	...
	if (dev->udev == NULL) {
		retval = -ENODEV;
		err("modem_hsic_read : No device or device unplugged %d", retval);
		goto exit;
	}

	/* if we cannot read at all, return EOF */
	if (!count){
		printk(KERN_DEBUG "modem_hsic_read :read request of 0 bytes  \n");
		return 0;
	}

	if (!dev->interface) {		/* disconnect() was called */
		retval = -ENODEV;
		err("modem_hsic_read : disconnect %d", retval);
		goto exit;
	}
	...
	if (!dev->processed_urb) {
		...
		if (retval < 0){
			printk(KERN_DEBUG "modem_hsic_read : modem_do_read_io2 , retval = =%d \n",retval); 			
			goto exit;
		}
		else 
		{
			printk(KERN_DEBUG "modem_hsic_read : modem_do_read_io3 , retval = =%d \n",retval);			
		}
	}
...
retry:
	...
	if (dev->bulk_in_filled) {
		...
		if (copy_to_user(buffer, dev->bulk_in_buffer, dev->bulk_in_filled)){
			retval = -EFAULT;
			printk(KERN_DEBUG "modem_hsic_read : copy_to_user retval =%d\n", retval);
			dev->bulk_in_filled = 0;
			goto exit;
		}
		else{
			retval = dev->bulk_in_filled;
			printk(KERN_DEBUG "modem_hsic_read : copy_to_user bytes_read =%d\n", dev->bulk_in_filled);
			dev->bulk_in_filled = 0;
			goto success;
		}
	} 
 ...
 *	modem_hsic_write
 */
static ssize_t modem_hsic_write(struct file *file, const char __user *user_buffer,
			        size_t count, loff_t *ppos)
{
	...
	spin_unlock_irq(&dev->err_lock);
	if (retval < 0){
		err("modem_hsic_write: misc error retval = %d",retval );
		goto error;
	}

	/* create a urb, and a buffer for BULK OUT, and copy the data to the urb */
	urb = usb_alloc_urb(0, GFP_KERNEL);
	if (!urb) {
		retval = -ENOMEM;
		err("modem_hsic_write: urb alloc retval = %d",retval );
		goto error;
	}
	...
	if (!buf) {
		retval = -ENOMEM;
		err("modem_hsic_write: usb_alloc_coherent retval = %d",retval );
		goto error;
	}

	if (copy_from_user(buf, user_buffer, writesize)) {		
		retval = -EFAULT;
		err("modem_hsic_write: copy_from_user retval = %d",retval );
		goto error;
	}
	...
	usb_fill_bulk_urb(urb, dev->udev,
			  ...
			  buf, writesize, modem_write_bulk_callback, dev);
...
static void boot_hsic_disconnect(struct usb_interface *interface)
{
	...
	dev_info(&interface->dev, "USB Hakuya Renesas Modem #%d now disconnected", minor);
}
